defmodule Derailed.Session do
  ...
  def init({id, user_id, ws_pid, token}) do
    {:ok,
     %{
       id: id,
       user_id: user_id,
  ...
  def handle_cast(:start, state) do
    ...
    {:ok, unify_pid} = GenRegistry.lookup_or_start(Derailed.Session.Registry, state.user_id, [state.user_id])
    Derailed.Session.Registry.subscribe(unify_pid, self())

    Logger.debug("[sessions-#{inspect(state.id)}] Successfully connected session to Registry")
