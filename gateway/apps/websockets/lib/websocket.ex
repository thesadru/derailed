defmodule Derailed.WebSocket do
  ...
  defp authorize_token(token) do
    ...
    user_id = Derailed.Token.get_device_id(token)

    case Postgrex.prepare_execute(:db, "get_user_by_id", "SELECT * FROM users WHERE id = $1;", [
           user_id
         ]) do
      {:ok, _query, result} ->
  ...
  def websocket_info(message, state) do
    ...
    case t do
      "USER_DELETE" ->
        ...
        GenRegistry.stop(Derailed.Session.Registry, state.user_id)
         ...
         {:ok, session_pid} ->
           if not Derailed.Session.is_token?(session_pid, message.token) do
             ...
             case Derailed.Session.resume(session_pid) do
               :ok ->
                 {:ok, %{state | session_id: message.session_id, session_pid: session_pid, identified: true}}
